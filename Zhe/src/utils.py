...
def plotTrainAndTestAccuracy(trainAccuracy, testAccuracy, filePath):
  plt.figure()
  plt.xlabel('epochs')
  plt.ylabel('accuracy')
  plt.ylim([0.0, 1.0])
  plt.plot(trainAccuracy, color='#FF0000', label='Trainning Data')
  plt.plot(testAccuracy, color='#0000FF', label='Test Data')
  plt.legend()
  plt.savefig(filePath)


def printSomeDatasetSamples(images, labels, filePath):
  fig, subplots = plt.subplots(5, 5)

  for x in range(5):
    for y in range(5):
      imageIndex = random.randint(0, len(images) - 1)
      classIndex = labels[imageIndex].tolist().index(1.0)
      subplots[x, y].axis('off')
      subplots[x, y].title.set_text(str(classIndex))
      subplots[x, y].imshow(images[imageIndex], cmap=plt.get_cmap('gray'))
