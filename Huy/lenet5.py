...
# Controls
SHOULD_USE_ARTIFICIAL_DATA = True
...
def load_dataset():
  ...
  if SHOULD_USE_ARTIFICIAL_DATA:
    ...
    artificial_data = image_dataset_from_directory('datasets/artificial_mnist/images', batch_size=1024, image_size=(28, 28), color_mode='grayscale')
    x_train = []
    y_train = []
    ...
    test_data = image_dataset_from_directory('datasets/test_mnist', batch_size=1024, image_size=(28, 28), color_mode='grayscale')
    x_test = []
    y_test = []
...
es = LossLogCallback(x_test, y_test, 'output/artificial_loss_log.txt', 'output/artificial_loss_plot', 'output/artificial_accuracy_plot', patience=10)
model.fit(x_train,
          y_train,
          epochs = 100,
...
x_test_by_class = [[] for i in range(10)]
y_test_by_class = [[] for i in range(10)]
for i in range(len(x_test)):
